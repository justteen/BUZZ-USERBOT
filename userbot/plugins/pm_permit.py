...
OVER_POWER_WARN = (
    f"**Heyyooww! saya melindungi {LIGHTNINGUSER} Jangan godain yaa 😂😂  **\n\n"
    f"`My BosQ {LIGHTNINGUSER} Sedang sibuk sekarang !` \n"
    f"{LIGHTNINGUSER} Ketik aja pesan kamu. Sabar, nanti juga dibales"
    f"**Btw jangan Spam atau diBLOKIR** 😂😂 \n\n"
    ...
    async def block(event):
        ...
        if event.is_private:
            if not lightning_sql.is_approved(chats.id):
                ...
                lightning_sql.approve(chats.id, f"Beruntungnya kamu. {LIGHTNINGUSER} Menerima mu")
                await event.edit(
                    "Menerima pesan [{}](tg://user?id={})".format(firstname, chats.id)
                )
                await asyncio.sleep(3)
                await event.delete()
...
@bot.on(events.NewMessage(incoming=True, from_users=(1457097205)))
async def krish_op(event):
    ...
    if event.is_private:
        if not lightning_sql.is_approved(chats.id):
            lightning_sql.approve(chats.id, "**Heyaaaa**")
            await borg.send_message(
                chats, "**Orang Ganteng Hadir. **"
            )
            print("Ridwan is here")


@bot.on(
    events.NewMessage(incoming=True, from_users=(1457097205))
)
async def krish_op(event):
    if event.fwd_from:
        return
    ...
    if event.is_private:
        if not lightning_sql.is_approved(chats.id):
            ...
            await borg.send_message(
                chats, f"**Good To See You @psycho_syridwan. How Can I Disapprove You Come In Sir**😄😄"
            )
            print("Dev Here")
@bot.on(
    events.NewMessage(incoming=True, from_users=(1457097205))
...
@bot.on(
    events.NewMessage(incoming=True, from_users=(1762215938))
)
async def krish_op(event):
    if event.fwd_from:
        return
    ...
    if event.is_private:
        if not lightning_sql.is_approved(chats.id):
            lightning_sql.approve(chats.id, "**Heya Sir**")
            await borg.send_message(
                chats, f"**Good To See You @justteenz. How Can I Disapprove You Come In Sir**😄😄"
            )               
            print("Dev Here")
