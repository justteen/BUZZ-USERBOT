...
NO_ADMIN = "Saya Bukan Admin Di Sini Niiba Ucapkan Stfu kepada orang yang ingin Anda bisukan"
NO_PERM = (
    "Saya tidak memiliki izin yang cukup! Ini sangat sed. Alexa bermain Tera Baap Aaya"
)
...
async def ban(bon):
    ...
    if reason:
        await bon.edit(f"kampret `{str(user.id)}` was banned !!\nReason: {reason}")
    else:
        await bon.edit(f"kampret `{str(user.id)}` was banned !!")
...
async def spider(spdr):
    ...
    if user.id == self_user.id:
        await spdr.edit("`Tangan terlalu pendek, tidak bisa lakban sendiri...\n(ヘ･_･)ヘ┳━┳`")
        return
...
async def get_users(show):
    ...
    try:
        ...
        await show.edit("Dang, this is a huge group. Uploading users lists as file.")
