...
LIGHTNINGUSER = str(ALIVE_NAME) if ALIVE_NAME else "⚡ 𝐁 𝐔 𝐙 𝐙 ⚡"
...
if BOT_MSG is None:
    BOT_LIT = f"Hey! This is adavanced PM Protection by [⚡ 𝐁 𝐔 𝐙 𝐙 ⚡](https://t.me/ossuport). "
else:
    BOT_LIT = BOT_MSG   
...
if LIGHTNING_WARN is None:
    WARNING = (
Translated from en to id.
    f"**{BOT_LIT}"
    f"** Saya di sini untuk melindungi PM {LIGHTNINGUSER} dari spamming.**\n\n"
    f"**Tuanku {LIGHTNINGUSER} sedang sibuk sekarang !** \n"
    f"Tolong beri tahu saya mengapa Anda datang ke sini."
    f"**Pilih alasan yang Anda inginkan dari bawah.** \n\n"
    f"**Tapi jangan spam jika tidak, Anda akan diblokir**\n\n"
    f"**Pilih juga alasan Asli Anda Jika tidak, Anda akan diblokir [Jangan Meremehkan]**"
   )
else:
...
LIGHTNING_BOT_PIC = (
    ...
    else "https://telegra.ph/file/89542f076c98cc5b20c01.jpg"
)
    ...
    if lightning.query.user_id == bot.uid and query.startswith("**Black") or query.startswith("Black"):
        ...
        result = builder.article(
            ...
            buttons=[
                ...
                [Button.url("Commands Not Working🥺", "https://t.me/ossuport")],
        ...
        result = builder.photo(
            ...
            text=WARNING,
            buttons=[
                [custom.Button.inline("Wanna Spam Something?😉", data="BUZZ_is_here_cant_spam")],
                [
...
async def lightning_pugins_query_hndlr(lightning):
    if not lightning.query.user_id == bot.uid:
        how = "Do you Really Think This is for you? \nThen Make your own ⚡ 𝐁 𝐔 𝐙 𝐙 ⚡ Bot and don't poke your nose in mine"
    ...
    try:
        if light_pulu_name in CMD_HELP:
           ...
           lightning_is_best += "\n\n**In Case Any Problem**[⚡ 𝐁 𝐔 𝐙 𝐙 ⚡](https://t.me/ossuport)".format(light_pulu_name)
    ...
    if light_pulu_name in CMD_LIST:
           ...
           lightning_help_strin  = f"**🔺 NAME 🔺 :** `{light_pulu_name}` \n\n`{CMD_LIST[light_pulu_name]}`\n\n**Ask at @ossuport"
           lightning_is_best = lightning_help_strin 
           lightning_is_best += "\n\n**In Case Any Problem @ossuport** ".format(light_pulu_name)
    lightning_help_strin = f"**🔺 NAME 🔺 :** `{light_pulu_name}` \n\n`{CMD_LIST[light_pulu_name]}`\n\n**Ask at @ossuport"
    lightning_is_best = lightning_help_strin 
    lightning_is_best += "\n\n**In Case Any Problem** @ossuport ".format(light_pulu_name)    
    if len(lightning_is_best) >= 4096:
          ...
          await lightning.edit(
               ...
               link_preview=False,
               buttons=[
                [custom.Button.inline("🇸‌🇵‌🇪‌🇨‌🇮‌🇦‌🇱‌", data="RIDWAN")],
                [custom.Button.inline("buzz 💢", data="lighyear")]],
         )
    else:
           await lightning.edit(
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"buzz_is_here_cant_spam")))
async def lightning_is_better(lightning):
    ...
    text1 = f"LOL **You Think So You Can**😂😂\n\n**[LUL 😂](tg://user?id={lightning_id}) Bye, aku blok kamu.**😂😂"
    await lightning.edit("yeaaa.. bomat")
...
async def lightning_is_better(lightning):
    ...
    await lightning.get_chat()
    lightning_id = lightning.query.user_id
    await lightning.edit("Ohh.. kamu mau ngobrol sama BOS.\n\nPls tunggu yaa \n\n**Btw** **kamu bisa tunggu bosQ**")
    await asyncio.sleep(2)
    await lightning.edit(
        "siapa nama mu??", buttons= [
        [Button.inline("School", data="school")],
        [Button.inline("Tg Causal Friend", data="tg_okay")],
        ], 
    )
    light_text = "`Warning`- ❗️⚠️Jangan kirim pesan lagi. mohon menunggu!!!❗️⚠️"
